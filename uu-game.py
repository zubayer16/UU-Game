...
def removePiece(board, piece, mill_list, phase):
    ...
    """
    print()
    removeCoord = verifyFormat("Please input piece to remove in format rownr,columnnr: ")
    ...
    if (phase == 3):
        if board[removeCoord[0]][removeCoord[1]] == piece:
            ...
            print("Please choose a valid opponent piece")
...
def verifyMovePiece(board, chosenPieceCoord, moveCoord, phase):
    """
...
def verifyChoosenPiece(board, chosenPieceCoord, playerPiece):
...
def verifyMove(phase, board, Playerpiece):
    ...
    while not validMove:
        ...
        if (phase == 2):
            coord[0] = verifyFormat("Please input coordinates for the chosen piece to move in format rownr,columnnr: ")
            if (not verifyChoosenPiece(board, coord[0], Playerpiece)):
                continue
            coord[1] = verifyFormat("Please input coordinates for placement in format rownr,columnnr: ")

            validMove = verifyMovePiece(board, coord[0], coord[1], phase)
    ...
    if (phase == 1):
        print("The player places a piece on:" + str(coord))
        print()
    else:
        print(f"The player moves the piece at {coord[0]} to {coord[1]}")
        print()
...
# Finds all of the coordinates of a specific symbol
def findPieces(board, piece):
    coordinates = []
...
def printGameState(board, playerpiece, aipeice, playerPhase, AIPhase):
    playerpieces = findPieces(board, playerpiece)
    aipeices = findPieces(board, aipeice)
    print()
    print("The player is in phase: " + str(playerPhase))
    print("The player has pieces on these coordinates: " + str(playerpieces))
    print()
    print("The AI is in phase: " + str(AIPhase))
    print("The AI has pieces on these coordinates: " + str(aipeices))
    print()
...
def aiRemovePiece(board, playerPiece, mills_list, phase):
    ...
    if available:
        ...
        board[temp[index][0]][temp[index][1]] = '.'
        print(f"the AI removes the piece at {[temp[index][0], temp[index][1]]} ")
...
def aiMove(board, playerpiece, aiPiece, aiPhase, difficulty, rounds, AIMills, prevPrevAIMills, playerMills,
           prevPrevPlayerMills):
    """
    Makes the ai perform a move depending on the difficulity. Easy will always perform random moves meanwhile medium uses 1/4 smart moves and hard always smart moves, the rest are random.
    @param board: The current board
    @param playerpiece: The symbol representing the player
    @param aiPiece: The symbol representing the AI
    @param aiPhase: The current phase of the AI
...
def gameLoop(board, playerPiece, opponentPiece, difficulty):
    ...
    while (rounds < 200):
        printGameState(board, playerPiece, opponentPiece, playerPhase, aiPhase)
        rounds = rounds + 1
        ...
        if new_mill:
            aiMills = updateMillList(board, opponentPiece, aiMills, prevPrevAIMills)
    ...
    if new_mills:
        for mill in new_mills:
            if mill not in prev_mill_list:
                ...
                print(
                    "the player have made a mill but the same mill has been made in two consecutive turns so it does not count ")
                print()
...
def printRules():
    print("Description of Gameplay")
    print("----------------------------")
    print(
        "The objective of the game is simple: starting with 12 pieces, with the goal to reduce your opponent's pieces to just 2 pieces on the 10x10 playing board with the creation of mills. A human player plays against an AI (our game engine) on the terminal.\n")
    print("Mill Creation and Removal:")
    print(
        "If a player manages to place three of their pieces in a row, either horizontally or vertically, you create a mill. This achievement grants the player the power to remove one of the opponent’s pieces, starting with loose pieces (pieces not part of any mill) if there are any. If no loose pieces exist, the player can remove a piece from one mill. Players cannot create the same mill on two consecutive turns. Mills can be created during all phases of the game.\n")
    print("The game unfolds in three phases for the player:")
    print("Phase 1: The player and the AI take turns placing their 12 pieces on available intersections on the board.")
    print(
        "Phase 2: When the player or AI has placed all their pieces, they enter phase two. In this phase, one can move their pieces on the board to available adjacent intersections during their turn.")
    print(
        "Phase 3: When a player or AI has just three pieces left, they enter the final and third phase. In this phase, the player that only has three pieces left can move their pieces to any available intersection on their turn.")
    print()
